...
describe('skills — spec validation', () => {
  ...
  test('description must be 1-1024, non-empty, no xml', () => {
    ...
    expect(() => validateSkillDescription('Extract text and tables from PDF files. Use when working with PDFs.')).not.toThrow()
  })
})
...
describe('skills — pure lib with org scoping, DTO string safety, no indexes', () => {
  test('stores organizationId/createdBy/editedBy as ObjectId, exposes as strings', async () => {
    const dto = await createSkill(db, { name: 'pdf-processing', description: 'Extract text and tables from PDFs. Use when working with PDFs.', content: '# PDF Skill\nInstructions...' }, { userId: user1, organizationId: orgA })
  ...
  test('rejects invalid name/description on create', async () => {
    await expect(createSkill(db, { name: 'Bad_Name', description: 'valid desc', content: 'body' }, { userId: user1, organizationId: orgA })).rejects.toThrow()
    await expect(createSkill(db, { name: 'valid-name', description: '', content: 'body' }, { userId: user1, organizationId: orgA })).rejects.toThrow()
  })

  test('getSkills is org-scoped', async () => {
    await createSkill(db, { name: 'skill-a1', description: 'desc a1 use when a', content: 'c' }, { userId: user1, organizationId: orgA })
    await createSkill(db, { name: 'skill-a2', description: 'desc a2 use when a', content: 'c' }, { userId: user1, organizationId: orgA })
    await createSkill(db, { name: 'skill-b1', description: 'desc b1 use when b', content: 'c' }, { userId: user2, organizationId: orgB })
  ...
  test('getSkillById prevents cross-org access', async () => {
    const dto = await createSkill(db, { name: 'secret-skill', description: 'secret desc use when secret', content: 'c' }, { userId: user1, organizationId: orgA })
  ...
  test('updateSkill validates spec and is org-scoped', async () => {
    const dto = await createSkill(db, { name: 'old-skill', description: 'old desc use when old', content: 'c' }, { userId: user1, organizationId: orgA })
    await expect(updateSkill(db, dto._id, { name: 'Invalid_Name' }, { userId: user2, organizationId: orgA })).rejects.toThrow()
    const updated = await updateSkill(db, dto._id, { name: 'new-skill' }, { userId: user2, organizationId: orgA })
    expect(updated?.name).toBe('new-skill')
    expect(updated?.editedBy).toBe(user2.toHexString())
    expect(await updateSkill(db, dto._id, { name: 'hack' }, { userId: user2, organizationId: orgB })).toBeNull()
  })

  test('deleteSkill is org-scoped', async () => {
    const dto = await createSkill(db, { name: 'to-del', description: 'to del desc use when del', content: 'c' }, { userId: user1, organizationId: orgA })
  ...
  test('repository binds org correctly', async () => {
    ...
    await repoA.createSkill({ name: 'repo-skill', description: 'repo desc use when repo', content: 'c' }, user1)
    expect((await repoA.getSkills()).length).toBe(1)
    expect((await repoB.getSkills()).length).toBe(0)
  })

  test('no indexes beyond _id', async () => {
    await createSkill(db, { name: 'idx-skill', description: 'idx desc use when idx', content: 'c' }, { userId: user1, organizationId: orgA })
