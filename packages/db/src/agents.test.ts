...
describe('agents — pure lib with org scoping, DTO string safety, no indexes', () => {
  test('stores organizationId/createdBy/editedBy as ObjectId, exposes as strings', async () => {
    const dto = await createAgent(
      db,
      { name: 'Jarvis', description: 'helper', content: 'you are helpful' },
  ...
  test('getAgents is org-scoped', async () => {
    await createAgent(
      db,
      { name: 'A1', description: 'd', content: 'c' },
      { userId: user1, organizationId: orgA }
    );
    await createAgent(
      db,
      { name: 'A2', description: 'd', content: 'c' },
      { userId: user1, organizationId: orgA }
    );
    await createAgent(
      db,
      { name: 'B1', description: 'd', content: 'c' },
  ...
  test('getAgentById prevents cross-org access', async () => {
    const dto = await createAgent(
      db,
      { name: 'Secret', description: 'd', content: 'c' },
      { userId: user1, organizationId: orgA }
    );
  ...
  test('updateAgent is org-scoped and updates editedBy/editedAt', async () => {
    const dto = await createAgent(
      db,
      { name: 'Old', description: 'd', content: 'c' },
      { userId: user1, organizationId: orgA }
    );
    ...
    expect(
      await updateAgent(db, dto._id, { name: 'Hack' }, { userId: user2, organizationId: orgB })
    ).toBeNull();
  ...
  test('deleteAgent is org-scoped', async () => {
    const dto = await createAgent(
      db,
      { name: 'ToDel', description: 'd', content: 'c' },
      { userId: user1, organizationId: orgA }
    );
  ...
  test('no indexes are created', async () => {
    await createAgent(
      db,
      { name: 'Idx', description: 'd', content: 'c' },
      { userId: user1, organizationId: orgA }
    );
