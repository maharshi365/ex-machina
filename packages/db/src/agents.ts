...
function getCollection(db: Db): Collection<Agent> {
  return db.collection<Agent>(AGENTS_COLLECTION)
}
...
export async function createAgent(
  ...
  const result = await getCollection(db).insertOne(doc as Agent)
