...
export const getActiveOrganizationServerFn = createServerFn({ method: 'GET' }).handler(
  async (): Promise<OrganizationDTO | null> => {
    ...
    try {
      const orgs = (await auth.api.listOrganizations({ headers: request.headers })) as unknown as
        | OrganizationDTO[]
        | null;
...
export const listOrganizationsServerFn = createServerFn({ method: 'GET' }).handler(
